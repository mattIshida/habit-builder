...
function Feed(){
    ...
    if(isSuccess){
        ...
        const feedCards = feed.filter(x=> !(x.type=='attempt' && x.content.success===null)).sort((a,b)=>b.content.created_at.localeCompare(a.content.created_at)).map(item => <FeedCard key={`${item.type}-${item.content.id}`} item={item} />)
        content = feedCards
